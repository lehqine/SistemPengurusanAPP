...
if ($stmt = $con->prepare("SELECT `APPLICATION_ID`, `TARIKH`, `MASA`, `STATUS`, lecturer.`LECTURER_ID`, `KATEGORI`, `NAMA`, appapplication.UNIVERSITI, appapplication.`KELAYAKAN_AKADEMIK`, appapplication.`PENGALAMAN`, appapplication.`PENGANUGERAHAN`, GELARAN, NO_KP, ALAMAT, POSKOD, NEGERI, DAERAH, FAKULTI, BIDANG, NO_TELEFON, NO_TELEFON_PEJABAT FROM `appapplication` LEFT JOIN `lecturer` ON appapplication.LECTURER_ID = lecturer.LECTURER_ID WHERE `STATUS` = 'SEDANG DIPROSES' AND appapplication.LECTURER_ID = '$id'")) {
  ...
  while (mysqli_stmt_fetch($stmt)) {
    array_push($list_of_application, array($application_id, $tarikh, $masa, $status, $lecturer_id, $kategori, $nama, $universiti, $kelayakan_akademik, $pengalaman, $penganugerahan, $gelaran, $no_kp, $alamat, $poskod, $negeri, $daerah, $fakulti, $bidang, $no_telefon, $no_telefon_pejabat));
  }
...
$kelayakan_akademik_arr = explode("^", $list_of_application[0][8]);
$empty_str = array_shift($kelayakan_akademik_arr);
$penganugerahan_arr = explode("^", $list_of_application[0][10]);
$empty_str = array_shift($penganugerahan_arr);
$pengalaman_arr = explode("^", $list_of_application[0][9]);
...
<body>
  ...
  <div class="main-body">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" autocomplete="off"
      class="sign-in-form">
      <?php
      include("./form_show_permohonan_details.php");
      ?>

      <?php
      echo "               <a href=\"../functions/application_action.php?action=ACCEPT&id=$aid&lecturer_id=$lid\" class=\"inline-btn\">TERIMA</a>
<br>
         <a href=\"../functions/application_action.php?action=REJECT&id=$aid&lecturer_id=$lid\" class=\"inline-btn\">TOLAK</a>
            </div>";
