...
//  $no_tel_pejabat = $_POST["no-tel-pejabat"];
  ...
  if (file_exists($_FILES['profil-img']['tmp_name']) || is_uploaded_file($_FILES['profil-img']['tmp_name'])) {
    $stmt1 = "UPDATE userfakulti SET `URL_AVATAR` = '$url_avatar', `NO_KP` = '$no_kad_pengenalan', `NAMA` = '$nama', `ALAMAT` = '$alamat_tempat_bekerja', `NO_TELEFON` = '$no_tel_bimbit' WHERE `USERFAKULTI_ID` = '$id'";
  } else {
    $stmt1 = "UPDATE userfakulti SET `NO_KP` = '$no_kad_pengenalan', `NAMA` = '$nama', `ALAMAT` = '$alamat_tempat_bekerja', `NO_TELEFON` = '$no_tel_bimbit' WHERE `USERFAKULTI_ID` = '$id'";
  }
...
if ($stmt = $con->prepare("SELECT `USERFAKULTI_ID`, `NAMA`, `PASSWORD`, `EMEL`, `NO_KP`, `ALAMAT`, `NO_TELEFON`, `URL_AVATAR` FROM userfakulti WHERE USERFAKULTI_ID = '$id' ")) {
  $stmt->execute();
  mysqli_stmt_bind_result($stmt, $userfakulti_id, $nama, $password, $emel, $no_kp, $alamat, $no_telefon, $url_avatar);
  while (mysqli_stmt_fetch($stmt)) {
    array_push($user, array($userfakulti_id, $nama, $password, $emel, $no_kp, $alamat, $no_telefon, $url_avatar));
  }
} else {
  // Something is wrong with the SQL statement, so you must check to make sure your accounts table exists with all 3 fields.
  echo 'Could not prepare statement!';
...
if (isset($_POST["change_password"])) {
  ...
  if($password1 == $user[0][2]){
    if ($stmt = $con->prepare("UPDATE userfakulti SET `PASSWORD` = '$password2'  WHERE `USERFAKULTI_ID` = '$id'")) {
      $stmt->execute();
...
<body>
  ...
  <div class="main-body">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" autocomplete="off"
      ...
      <?php
      include("./form_profil_userfakulti.php");
      ?>
