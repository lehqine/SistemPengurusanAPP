...
if ($stmt = $con->prepare("SELECT `NOTI_ID`, `APP_ID`, `TEXT`, `TARIKH`, `MASA`, `READ_NOTI` FROM `app_noti` where APP_ID = '$id' ORDER BY `READ_NOTI`, `TARIKH` DESC, `MASA` DESC")) {
...
if ($stmt = $con->prepare("UPDATE `app_noti` SET `READ_NOTI`='T' where APP_ID = '$id'")) {
