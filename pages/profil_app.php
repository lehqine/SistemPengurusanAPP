...
if (isset($_POST['submit'])) {
  ...
  if (file_exists($_FILES['profil-img']['tmp_name']) || is_uploaded_file($_FILES['profil-img']['tmp_name'])) {
    $stmt1 = "UPDATE app SET `URL_AVATAR` = '$url_avatar', `FAKULTI` = '$fakulti', `BIDANG` = '$bidang', `NO_KP` = '$no_kad_pengenalan', `NAMA` = '$nama', `ALAMAT` = '$alamat_tempat_bekerja', `NO_TELEFON` = '$no_tel_bimbit' WHERE `APP_ID` = '$id'";
  } else {
    $stmt1 = "UPDATE app SET `FAKULTI` = '$fakulti', `BIDANG` = '$bidang', `NO_KP` = '$no_kad_pengenalan', `NAMA` = '$nama', `ALAMAT` = '$alamat_tempat_bekerja', `NO_TELEFON` = '$no_tel_bimbit' WHERE `APP_ID` = '$id'";
  }
  ...
  $stmt1 = "UPDATE lecturer SET `KELAYAKAN_AKADEMIK` = '$kelayakan_akademik', `PENGALAMAN` = '$pengalaman', `PENGANUGERAHAN` = '$penganugerahan', `GELARAN` = '$gelaran', `UNIVERSITI` = '$universiti', `POSKOD` = '$poskod', `DAERAH` = '$daerah', `NEGERI` = '$negeri', `NO_TELEFON_PEJABAT` = '$no_tel_pejabat' WHERE `LECTURER_ID` = '$lid'";

  //`GELARAN`, a.`UNIVERSITI`, `POSKOD`, `DAERAH`, `NEGERI`, a.`NO_TELEFON_PEJABAT`, a.`KELAYAKAN_AKADEMIK`, a.`PENGALAMAN`, a.`PENGANUGERAHAN`
  if ($stmt = $con->prepare($stmt1)) {
...
if ($stmt = $con->prepare("SELECT `GELARAN`, a.`UNIVERSITI`, `POSKOD`, `DAERAH`, `NEGERI`, a.`NO_TELEFON_PEJABAT`, a.`KELAYAKAN_AKADEMIK`, a.`PENGALAMAN`, a.`PENGANUGERAHAN`, a.LECTURER_ID FROM lecturer a LEFT JOIN appapplication b ON a.LECTURER_ID = b.LECTURER_ID LEFT JOIN app c ON c.APPLICATION_ID= b.APPLICATION_ID WHERE c.APP_ID = '$id' ")) {
  $stmt->execute();
  mysqli_stmt_bind_result($stmt, $gelaran, $universiti, $poskod, $daerah, $negeri, $no_telefon, $kelayakan_akademik, $pengalaman, $penganugerahan, $llid);
  while (mysqli_stmt_fetch($stmt)) {
    array_push($user_2, array($gelaran, $universiti, $poskod, $daerah, $negeri, $no_telefon, $kelayakan_akademik, $pengalaman, $penganugerahan, $llid));
  }

  $kelayakan_akademik_arr = explode("^", $user_2[0][6]);
  $empty_str = array_shift($kelayakan_akademik_arr);
  $penganugerahan_arr = explode("^", $user_2[0][8]);
  $empty_str = array_shift($penganugerahan_arr);
  $pengalaman_arr = explode("^", $user_2[0][7]);
...
if (isset($_POST["change_password"])) {
  ...
  if ($password1 == $user[0][8]) {
    if ($stmt = $con->prepare("UPDATE app SET `PASSWORD` = '$password2'  WHERE `APP_ID` = '$id'")) {
      $stmt->execute();
