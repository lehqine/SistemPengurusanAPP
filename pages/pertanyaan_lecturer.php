...
if (isset($_POST['submit'])) {
   ...
   if (
    ...
    ('$today_date', '$perkara', '$ringkasan', 'SEDANG DIPROSES', '', 'lecturer', '$id')")
   ) {
...
if ($stmt = $con->prepare("SELECT `PERTANYAAN_ID`, `TARIKH`, `PERKARA`, `RINGKASAN`, `PERTANYAAN_STATUS`, `TINDAKAN`  FROM `pertanyaan` WHERE `ID` = '$id' AND `JENIS` = 'lecturer' ORDER BY `TARIKH` DESC")) {
...
if ($stmt = $con->prepare("SELECT `PERTANYAAN_ID`, `TARIKH`, `PERKARA`, `RINGKASAN`, `PERTANYAAN_STATUS`, `TINDAKAN`  FROM `pertanyaan` WHERE `ID` = '$id' AND `JENIS` = 'lecturer'  ORDER BY `TARIKH` DESC LIMIT $page_first_result, $results_per_page")) {
...
<?php for($page = 1; $page<= $number_of_page; $page++) {
   echo '<a href = "./pertanyaan_lecturer.php?page=' . $page . '">' . $page . ' </a>';
}?>
</div>
